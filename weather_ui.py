...
with col_top[0]:
	...
	st.markdown(f"Feels like {current_apparent_temperature:.0f}°C<br><b>{now.time().strftime("%H:%M %p")}</b>", 
...
with col_bot[0]:
	...
	h_data["Time"] = pd.to_datetime(h_data["Time"]).dt.time
	...
	variable = st.selectbox(label="Variable", options=var_list)
	figure = px.scatter(h_data, x=h_data.index, y=variable, title=f"24-Hour {variable} Forecast for {city}")
